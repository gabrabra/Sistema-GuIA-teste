...
const defaultClassifyInstructions = `### ROLE
...
const defaultGuiaRespondeGeralInstructions = `✅ SYSTEM PROMPT – GuIA Responde – Geral (VERSÃO DEFINITIVA BLINDADA + TURNO ÚNICO)
...
const defaultGuiaRespondePortuguesInstructions = `INÍCIO DO SYSTEM PROMPT
...
export const runWorkflow = async (workflow: WorkflowInput) => {
  return await withTrace("1-GuIA Responde-4o-mini", async () => {
    ...
    let classifyInstructions = defaultClassifyInstructions;
    ...
    try {
      ...
      if (agentsMap['classify']) {
        classifyInstructions = agentsMap['classify'].instructions;
        classifyModel = agentsMap['classify'].model;
      } else {
        await pool.query(
          `INSERT INTO ai_agents (id, name, instructions, model) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING`,
          ['classify', 'Classify', defaultClassifyInstructions, defaultModel]
        );
      }
    ...
    const guiaRespondeGeral = new Agent({
      ...
      instructions: geralInstructions,
      model: geralModel,
      tools: [
        fileSearch
      ],
    ...
    const guiaRespondePortuguS4oMini = new Agent({
      ...
      instructions: portuguesInstructions,
